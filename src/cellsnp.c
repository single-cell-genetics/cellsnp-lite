// cellsnp.c - cellsnp main function (cmdline interface).
...
static void print_usage(FILE *fp) {
    ...
    fprintf(fp, "\n");
    fprintf(fp, "Options:\n");
    fprintf(fp, "  -s, --samFile STR        Indexed BAM/CRAM file(s), comma separated multiple samples.\n");
    fprintf(fp, "  -S, --samFileList FILE   A file listing BAM/CRAM files, each per line.\n");
    ...
    fprintf(fp, "  -b, --barcodeFile FILE   A plain file listing all effective cell barcodes.\n");
    ...
    fprintf(fp, "  -p, --nproc INT      Number of threads [%d]\n", CSP_NTHREAD);
    ...
    fprintf(fp, "  --minCOUNT INT       Minimum aggragated UMI or read count [%d]\n", CSP_MIN_COUNT);
    ...
    fprintf(fp, "  --maxDEPTH INT       At a position, read maximally INT reads per input file, to avoid\n");
    fprintf(fp, "                       excessive memory usage; 0 means highest possible value [%d]\n", CSP_MAX_DEPTH);
    fprintf(fp, "  --countORPHAN        If use, do not skip anomalous read pairs.\n");
    fprintf(fp, "\n");
    fprintf(fp, "For details of these options, please checkout the manual at:\n");
    fprintf(fp, "https://cellsnp-lite.readthedocs.io/en/latest/main/manual.html#full-parameters\n");
    fprintf(fp, "\n");
