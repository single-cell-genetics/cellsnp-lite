...
bin_csp2mtx=$root_dir/run/bm2_bm3/snp/csp2mtx.py
...
cmd="$bin_bcftools view -R $vcf0 --threads $ncores -Ou $vcf1 | \\
     $bin_bcftools sort -m 8G -o $vcf1_sort"
eval_cmd "$cmd" "$part_aim" "$out_log" "$err_log"

part_aim="csp2mtx"
cmd="$bin_python $bin_csp2mtx --vcf0 $vcf0_sort --vcf1 $vcf1_sort --outdir $out_dir"
eval_cmd "$cmd" "$part_aim" "$out_log" "$err_log"
