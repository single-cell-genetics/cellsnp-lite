...
run_name=bm2_bm3
bin_mode2=$root_dir/run/bm2_bm3/bm2_bm3.sh
...
for nc in $multi_cores; do
    ...
    out_log=$log_dir/`basename $bin_mode2`.ncores${nc}.out.log
    err_log=$log_dir/`basename $bin_mode2`.ncores${nc}.err.log
    no_dup_opt=""
    if [ $no_dup -eq 1 ]; then no_dup_opt="--no-dup"; fi
    primary_aln_opt=""
    if [ $primary_aln -eq 1 ]; then primary_aln_opt="--primary"; fi
    cmd="qsub -q $qsub_qname -N bm2_bm3_ncores${nc} \
        -l nodes=$qsub_nodes:ppn=$[nc + 1],mem=${qsub_mem}gb,walltime=${qsub_walltime}:00:00 \
        -o $out_log -e $err_log -- \\
        $bin_mode2 -O $res_dir -f $perf_file -n $nrep -p $nc -s $intval --bam $bam_file --snp $snp_file \
