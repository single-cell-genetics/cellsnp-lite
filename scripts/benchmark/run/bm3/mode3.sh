#!/bin/bash 
#this script is aimed to run different softwares for mode 3.
#hxj5<hxj5@hku.hk>
...
all_tools="bcftools-R bcftools-T cellSNP cellsnp-lite-R cellsnp-lite-T"
...
echo "=> ABSTRACT this script is aimed to run different softwares for mode 3."
    ...
    for i in `seq 1 $nrep`; do
        ...
        if [ $? -eq 1 ]; then 
            ...
            cmd="$bin_bcftools mpileup -b $bam_file -d $max_depth -f $fasta_file -q $min_mapq \
                   -Q $min_bq --incl-flags $incl_flag --excl-flags $excl_flag -R $snp_file -a $anno_tag \
                   -I --threads $nc -Ou | \\
        ...
        if [ $? -eq 1 ]; then 
            ...
            echo "#!/bin/bash" > $bcft_script
            cmd="$bin_bcftools mpileup -b $bam_file -d $max_depth -f $fasta_file -q $min_mapq \
                   -Q $min_bq --incl-flags $incl_flag --excl-flags $excl_flag -T $snp_file -a $anno_tag \
                   -I --threads $nc -Ou | \\
                   $bin_bcftools view -i 'INFO/DP > 0' -V indels --threads $nc -Oz -o $res_dir/bcftools.vcf.gz"
            echo "$cmd" >> $bcft_script
            chmod u+x $bcft_script
        ...
        if [ $? -eq 1 ]; then 
            ...
            res_dir=$out_dir/${app}_ncores${nc}_rep${i}
            safe_mkdir $res_dir
            cmd="$bin_memusg -t -H $bin_cellsnp -S $bam_file -I $sample_lst -O $res_dir -R $snp_file -p $nc \
        ...
        is_in cellsnp-lite-R "$tools"
        if [ $? -eq 1 ]; then 
            app=cellsnp-lite-R
            res_dir=$out_dir/${app}_ncores${nc}_rep${i}
            safe_mkdir $res_dir
            cmd="$bin_memusg -t -H $bin_cellsnp_lite -S $bam_file -i $sample_file -O $res_dir -R $snp_file -p $nc \
                     --cellTAG $cell_tag --UMItag $umi_tag --minCOUNT $min_count --minMAF $min_maf \
        ...
        is_in cellsnp-lite-T "$tools"
        if [ $? -eq 1 ]; then 
            app=cellsnp-lite-T
            res_dir=$out_dir/${app}_ncores${nc}_rep${i}
            safe_mkdir $res_dir
            cmd="$bin_memusg -t -H $bin_cellsnp_lite -S $bam_file -i $sample_file -O $res_dir -T $snp_file -p $nc \
                     --cellTAG $cell_tag --UMItag $umi_tag --minCOUNT $min_count --minMAF $min_maf \
                     --minLEN $min_len --minMAPQ $min_mapq --exclFLAG $excl_flag --inclFLAG $incl_flag --gzip --genotype"
            run_app $app "$cmd" $nc $i $res_dir $perf_file
            sleep $intval
