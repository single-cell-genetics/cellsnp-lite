...
# parse args
cmdline=`echo $0 $*`
ARGS=`getopt -o t:p:O:h --long bam:,snp:,sample:,fasta:,tools:,ncores:,rootdir:,outdir:,remove-tmp,help -n "" -- "$@"`
if [ $? -ne 0 ]; then
...
for nc in $multi_cores; do
    ...
    out_log=$log_dir/`basename $bin_mode3`.ncores${nc}.out.log
    err_log=$log_dir/`basename $bin_mode3`.ncores${nc}.err.log
    no_dup_opt=""
    if [ $no_dup -eq 1 ]; then no_dup_opt="--no-dup"; fi
    primary_aln_opt=""
    if [ $primary_aln -eq 1 ]; then primary_aln_opt="--primary"; fi
    cmd="qsub -q $qsub_qname -N bm3_ncores${nc} \
        -l nodes=$qsub_nodes:ppn=$[nc + 1],mem=${qsub_mem}gb,walltime=${qsub_walltime}:00:00 \
        -o $out_log -e $err_log -- \\
        $bin_mode3 -O $res_dir -f $perf_file -n $nrep -p $nc -s $intval --bam $bam_file --snp $snp_file \
            --sample $sample_file --fasta $fasta_file -t $tools --min-mapq $min_mapq --rootdir $root_dir \
