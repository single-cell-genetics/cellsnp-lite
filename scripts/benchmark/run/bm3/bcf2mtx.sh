#!/bin/bash
#this script is aimed to convert bcftools output to cellSNP-style matrices
#hxj5<hxj5@hku.hk>
...
bin_bcf2mtx=$script_dir/bm/bcf2mtx.py
...
echo "=> ABSTRACT this script is aimed to convert bcftools output to cellSNP-style matrices"
...
# core part
part_aim="sort vcf0"
cmd="$bin_bcftools sort -m 8G $vcf0 > $vcf0_sort"
eval_cmd "$cmd" "$part_aim" "$out_log" "$err_log"

part_aim="index vcf1"
if [ ! -f "${vcf1}.csi" ] && [ ! -f "${vcf1}.tbi" ]; then
    cmd="$bin_bcftools index --threads $ncores $vcf1" 
    eval_cmd "$cmd" "$part_aim" "$out_log" "$err_log"
fi

part_aim="filter vcf1"
cmd="$bin_bcftools view -i 'INFO/DP > 0' -R $vcf0 -V indels --threads $ncores -Ou $vcf1 | \\
     $bin_bcftools sort -m 8G -o $vcf1_sort"
eval_cmd "$cmd" "$part_aim" "$out_log" "$err_log"

part_aim="bcf2mtx"
cmd="$bin_python $bin_bcf2mtx --vcf0 $vcf0_sort --vcf1 $vcf1_sort --outdir $out_dir"
eval_cmd "$cmd" "$part_aim" "$out_log" "$err_log"
