...
# parse args
cmdline=`echo $0 $*`
ARGS=`getopt -o t:p:O:h --long bam:,fasta:,tools:,ncores:,rootdir:,outdir:,remove-tmp,help -n "" -- "$@"`
if [ $? -ne 0 ]; then
...
for nc in $multi_cores; do
    ...
    cmd="qsub -q $qsub_qname -N bm2_ncores${nc} \
        -l nodes=$qsub_nodes:ppn=$[nc + 1],mem=${qsub_mem}gb,walltime=${qsub_walltime}:00:00 \
        -o $out_log -e $err_log -- \\
        $bin_mode2 -O $res_dir -f $perf_file -n $nrep -p $nc -s $intval --bam $bam_file \
            --fasta $fasta_file -t $tools --min-mapq $min_mapq --rootdir $root_dir"
    echo >> $script_run
    echo "$cmd" >> $script_run
done
